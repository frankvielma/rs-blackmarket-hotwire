...
export default class extends Controller {
  ...
  validateEmail() {
    ...
    const isValid = emailRegex.test(admin_user_email.value);
    this.showValidationForTarget('email', isValid)
    return isValid
  }

  validatePassword() {
    const isValid = admin_user_password.value.length >= 8
    this.showValidationForTarget('password', isValid)
    return isValid
  }

  validateConfirmPassword() {
    const confirmPasswordInput = admin_user_password_confirmation.value;
    const isPasswordLengthValid = confirmPasswordInput.length >= 8;
    const doPasswordsMatch = admin_user_password.value === confirmPasswordInput;
